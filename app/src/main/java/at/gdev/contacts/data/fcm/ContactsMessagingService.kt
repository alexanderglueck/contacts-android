...
class ContactsMessagingService : FirebaseMessagingService() {
    ...
    /**
     * Fires with the current installation ID on registration and whenever the FID changes
     * (restore, reinstall, cleared data). Re-registers so the backend always holds a live
     * FID. This replaced onNewToken, which is not overridden any more: registration goes
     * through installation IDs, so no token is ever issued to refresh.
     */
